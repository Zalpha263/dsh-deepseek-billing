...
`;
		...
		const CONTRIBUTION = {
			...
			descriptors: [
				...
				{
					id: "dsh-deepseek-billing#deepseekBilling/setAlias",
					service: "deepseekBilling",
					namespace: "deepseekBilling",
					method: "setAlias",
					invocation: { kind: "direct" },
					parameters: [
						{ name: "input", wire: "input", source: "json", codec: strictCodec("dsh-deepseek-billing#deepseekBilling/setAlias:input") }
					],
					result: strictCodec("dsh-deepseek-billing#deepseekBilling/setAlias:result"),
					sourceLocation: { file: "dsh-deepseek-billing/lib/client.js", line: 1, column: 1 }
				}
			]
		};
		...
		function remote() {
			...
			return {
				...
				setAlias: (input) => call("setAlias", input)
			};
		}
		...
		/* v0.1.20：别名覆盖的本地持久化镜像（与 prices/rate 同模式，覆盖硬编码别名）。
		   overrides: model -> { canonical, label? }；removals: 被移除（含内置行）的 model。 */
		let aliasPersist = { overrides: Object.create(null), removals: [] };
		function persistAliases() {
			lsSet("aliases", aliasPersist);
		}
		function hydrateAliases(r) {
			const raw = lsGet("aliases", null);
			if (!raw || typeof raw !== "object") return Promise.resolve();
			const overrides = raw.overrides && typeof raw.overrides === "object" ? raw.overrides : {};
			const removals = Array.isArray(raw.removals) ? raw.removals.filter((m) => typeof m === "string" && m !== "") : [];
			const jobs = [];
			for (const model of Object.keys(overrides)) {
				const o = overrides[model];
				if (o && typeof o.canonical === "string" && o.canonical !== "") {
					jobs.push(r.setAlias({ model, canonical: o.canonical, label: o.label }));
				}
			}
			for (const model of removals) {
				jobs.push(r.setAlias({ model, remove: true }));
			}
			if (jobs.length > 0) {
				aliasPersist = { overrides: Object.assign(Object.create(null), overrides), removals };
				return Promise.all(jobs);
			}
			return Promise.resolve();
		}
		...
		function buildSkeleton(st) {
			...
			const costCard = h("div", { className: "pvcst-card" }, [
				...
				h("div", { className: "pvcst-foot", style: { marginTop: 4 }, text: "" })
			]);
			...
			const detailTitle = h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 } }, [
				h("div", { className: "pvcst-title", style: { marginBottom: 0, flex: 1 }, text: "Token 明细" }),
				refreshBtn
			]);
		...
		function renderData(st) {
			...
			if (callsInfo.aliased > 0) {
				const aliasRows = (d.byModel || []).filter(function (r) { return r.aliased; }).map(function (r) { return r.model; });
				note.push(callsInfo.aliased + " 次经别名通道（" + (aliasRows.join("、") || "配置别名") + "）调用，已按对应 DeepSeek 模型计价");
			}
			...
			if (calls.aliased > 0) notes.push(calls.aliased + " 次别名通道调用已计价");
			const meta = d.meta || {};
		...
		function renderForm(st) {
			...
			/* ---------- v0.1.20：别名通道（代理模型 → 官方模型） ----------
			   行数据来自宿主 summary.meta.aliases（内置 + 运行时覆盖的合并视图）；
			   覆盖/移除/重置均经 setAlias 落库并持久化到 localStorage，
			   替换掉原先只能写死在内置表中的行为。 */
			const aliasBox = h("div", { className: "pvcst-form", style: { marginTop: 10 } });
			aliasBox.appendChild(h("div", { className: "pvcst-label", style: { fontSize: 13, fontWeight: 600, marginBottom: 8, color: "var(--dsw-alias-label-primary)" }, text: "别名通道（代理模型 → 官方模型）" }));
			const aliasList = h("div");
			aliasBox.appendChild(aliasList);
			const aliasMsg = h("div", { className: "pvcst-foot", style: { marginTop: 4, flex: 1 } });
			const entryModelInput = h("input", { type: "text", className: "pvcst-input", placeholder: "代理 ID", style: { width: 110 } });
			const entryCanonInput = h("input", { type: "text", className: "pvcst-input", placeholder: "官方模型", style: { width: 170 } });
			function pollAliasRows() {
				if (!st.alive) return;
			...
			function renderAliasRows() {
				...
				const rows = (st.data && st.data.meta && Array.isArray(st.data.meta.aliases)) ? st.data.meta.aliases : [];
				if (rows.length === 0) {
					aliasList.appendChild(h("div", { className: "pvcst-foot", style: { marginBottom: 6 }, text: "暂无别名行（可在下方添加；内置别名在打开会话后显示）" }));
				}
				for (const entry of rows) {
					const input = h("input", { type: "text", className: "pvcst-input", value: entry.canonical, style: { width: 170 } });
					const saveBtn2 = h("button", { type: "button", className: "pvcst-btn", text: "保存" });
					const delBtn = h("button", { type: "button", className: "pvcst-btn", text: entry.builtin === true ? "移除" : "删除" });
					saveBtn2.addEventListener("click", function () {
						const v = input.value.trim();
						if (v === "") { aliasMsg.textContent = "官方模型不能为空"; return; }
						remote().setAlias({ model: entry.model, canonical: v }).then(function (r) {
							if (r && r.ok === false) { aliasMsg.textContent = "保存失败：" + String((r.error && r.error.message) || "非法映射"); return; }
							aliasPersist.overrides[entry.model] = { canonical: v };
							aliasPersist.removals = aliasPersist.removals.filter(function (x) { return x !== entry.model; });
							persistAliases();
							aliasMsg.textContent = "已保存（下次打开自动生效）";
						}).catch(function (e) { aliasMsg.textContent = "保存失败：" + String(e && e.message || e); });
					});
					delBtn.addEventListener("click", function () {
						remote().setAlias({ model: entry.model, remove: true }).then(function () {
							aliasPersist.removals.push(entry.model);
							aliasPersist.removals = aliasPersist.removals.filter(function (m, i, arr) { return arr.indexOf(m) === i; });
							delete aliasPersist.overrides[entry.model];
							persistAliases();
							aliasMsg.textContent = "已移除（下次打开自动生效）";
							renderAliasRows();
						}).catch(function (e) { aliasMsg.textContent = "移除失败：" + String(e && e.message || e); });
					});
					aliasList.appendChild(h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 6 } }, [
						h("span", { className: "pvcst-label", style: { fontSize: 12, width: 130, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, title: entry.model, text: entry.label || entry.model }),
						input, saveBtn2, delBtn
					]));
				}
				const addRow = h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginTop: 4 } }, [
					entryModelInput, entryCanonInput,
					h("button", { type: "button", className: "pvcst-btn", text: "添加" })
				]);
				addRow.children[2].addEventListener("click", function () {
					const m = entryModelInput.value.trim();
					const c = entryCanonInput.value.trim();
					if (m === "" || c === "") { aliasMsg.textContent = "代理 ID 与官方模型均不能为空"; return; }
					remote().setAlias({ model: m, canonical: c }).then(function (r) {
						if (r && r.ok === false) { aliasMsg.textContent = "添加失败：" + String((r.error && r.error.message) || "非法映射"); return; }
						aliasPersist.overrides[m] = { canonical: c };
						aliasPersist.removals = aliasPersist.removals.filter(function (x) { return x !== m; });
						persistAliases();
						entryModelInput.value = "";
						entryCanonInput.value = "";
						aliasMsg.textContent = "已添加（下次打开自动生效）";
						pollAliasRows();
					}).catch(function (e) { aliasMsg.textContent = "添加失败：" + String(e && e.message || e); });
				});
				aliasList.appendChild(addRow);
			}
			const aliasResetBtn = h("button", { type: "button", className: "pvcst-btn", text: "恢复内置别名" });
			aliasResetBtn.addEventListener("click", function () {
				remote().setAlias({ reset: true }).then(function () {
					aliasPersist = { overrides: Object.create(null), removals: [] };
					persistAliases();
					aliasMsg.textContent = "已恢复内置别名";
					pollAliasRows();
				}).catch(function (e) { aliasMsg.textContent = String(e && e.message || e); });
			});
			aliasBox.appendChild(h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginTop: 6 } }, [aliasResetBtn, aliasMsg]));
			renderAliasRows();
