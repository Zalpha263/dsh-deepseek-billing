...
`;
		...
		function renderData(st) {
			...
			if (callsInfo.unpriced > 0 && callsInfo.nonDeep > 0) {
				note.push(callsInfo.unpriced + " 次调用在 2026-08-17 峰谷定价前，另有 " + callsInfo.nonDeep + " 次非 DeepSeek 模型调用——均无价可计");
			} else if (callsInfo.unpriced > 0) {
				note.push(callsInfo.unpriced + " 次调用发生在 2026-08-17 峰谷定价前，无价可计（Token 已计入明细）");
			} else if (callsInfo.nonDeep > 0) {
				note.push(callsInfo.nonDeep + " 次调用为非 DeepSeek 模型，不计价");
			}
		...
		function mountPanel(el) {
			...
			const poll = function () {
				const arg = currentSessionId ? { sessionId: currentSessionId } : {};
				remote().summary(arg).then(function (r) {
					if (!st.alive) return;
					st.data = r;
					st.err = null;
					renderData(st);
