...
`;
		...
		function standaloneVisible() { return standalone.wantOpen === true && hostActive === false; }
		...
		function openStandalone() {
			...
			if (!hostActive) ensureStandalone();
			saveStandalone();
			bumpUi();
		}
		...
		/* sidebarPanel 服务出现/消失时在「右侧栏标签」与「独立浮动卡」之间切换 */
		function setHostActive(v) {
			if (hostActive === v) return;
			hostActive = v;
			if (v) hideStandalone();
		...
		async function apply(ctx) {
			...
			try {
				...
				/* 右侧栏标签面板（ui-beautify 可能晚于本插件提供 sidebarPanel）。
				   规范见 dsh-ui-beautify/docs/plugin-panel-integration.md（sidebarPanel API v1）。
				   v2.0.0 起 ui-beautify 不再自带「插件面板」宿主：注册进来的面板会成为
				   DSH **官方右侧栏**的一个标签页（标签条 / 浮动 / 分屏 / 每会话状态全部
				   由官方 dockkit 负责），服务名也从 `dock` 改成 `sidebarPanel`。
				   ① 首选 cordis 可选依赖 `ctx.inject(['sidebarPanel'], cb)`：服务出现时
				      执行回调（注册面板），消失（ui-beautify 热重载/重装）时执行回调返回的
				      清理函数，再次出现时自动重新执行 —— 不需要轮询，也不会因为重复的
				      internal/service 事件把用户正在看的面板反复注销重注册
				      （那会导致热重载后面板回来但是关着的）；
				   ② 守护 ctx（运行时动态包）没有 `inject`，退化为 internal/service 事件
				      + 1s 兜底轮询的**幂等**绑定器：已绑定且 `sidebarPanel.has(id)` 为真时
				      跳过，只在服务注销或 host 被换掉时才重绑。 */
				const panelDef = {
					id: PANEL_ID,
					title: PANEL_TITLE,
					icon: PANEL_ICON,
					/* 右侧栏「开始」引导页上的入口胶囊：没有它用户只能在标签条的 + 里找。 */
					entry: { title: PANEL_TITLE, description: "查看 DeepSeek 峰谷时段计费明细", order: 110 },
					mount: mountPanel
				};
				const registerInto = function (d) {
					try {
						const dispose = d.registerPanel(panelDef);
						debugLog("sidebarPanel 注册成功:", PANEL_ID);
						/* v0.1.23：面板已交给右侧栏标签 —— 独立浮动卡与标题栏入口自动让位 */
						setHostActive(true);
						return function () {
						...
						console.error("[dsh-deepseek-billing] sidebarPanel registration failed:", err);
						return null;
					}
				};
				const bindHostFallback = function () {
					let dispose = null;
					let bound = false;
					let stopped = false;
					...
					const sync = function () {
						if (stopped) return;
						const d = ctx.get("sidebarPanel");
						if (d === undefined || d === null || typeof d.registerPanel !== "function") { unbind(); return; }
						if (bound) {
							/* 幂等判据用 has()：不要用对象身份比较 —— 守护 ctx 下每次
							   get 都是新 Proxy。老宿主没有 has 时保守地不重绑。 */
							const alive = typeof d.has === "function" ? d.has(PANEL_ID) : true;
							if (alive) return;
							unbind();
						}
					...
					let off = null;
					try {
						off = ctx.on("internal/service", function (name) { if (name === "sidebarPanel") sync(); });
					} catch (err) {
				...
				if ("inject" in ctx) {
					try {
						/* 注意：回调**必须是箭头函数**（不可被 `new` 调用）。cordis 的
						   ...
						   —— 服务消失/本插件卸载时注册就泄漏在宿主里。 */
						ctx.inject(["sidebarPanel"], (hostCtx) => {
							const d = hostCtx.get("sidebarPanel");
				...
				ctx.effect(function standaloneLifecycle() {
					if (standalone.wantOpen === true && !hostActive) ensureStandalone();
					return function () {
