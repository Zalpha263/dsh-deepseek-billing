...
/* ===== 别名通道：同源 DeepSeek 模型经代理/适配层暴露的 ID（v0.1.20） =====
   内置行是上游维护者的环境默认（ECNU 校园代理）；运行时可用 setAlias 覆盖
   任意行的映射、添加新别名行、或移除内置行（含还原）。覆盖表经客户端
   localStorage 持久化，启动时重新灌入。不在表内的模型仍按「非 DeepSeek」处理。 */
const BUILTIN_ALIASES = Object.create(null)
BUILTIN_ALIASES['ecnu-max'] = { canonical: 'deepseek-v4-flash', label: 'V4 Flash · ECNU' }
BUILTIN_ALIASES['ecnu-image'] = { canonical: 'deepseek-v4-flash-vision-exp', label: 'V4 Flash Vision · ECNU' }
const aliasOverrides = Object.create(null) // model -> { canonical, label? }
const aliasRemovals = new Set() // model -> 移除（含内置行）
function resolveAlias(model) {
  if (typeof model !== 'string') return undefined
  if (aliasRemovals.has(model)) return undefined
  if (aliasOverrides[model] !== undefined) {
    const o = aliasOverrides[model]
    return { canonical: o.canonical, label: o.label, builtin: false }
  }
  const builtin = BUILTIN_ALIASES[model]
  return builtin === undefined ? undefined : { canonical: builtin.canonical, label: builtin.label, builtin: true }
}
function aliasEntries() {
  const out = []
  const seen = new Set()
  for (const model of Object.keys(BUILTIN_ALIASES)) {
    const alias = resolveAlias(model)
    if (alias !== undefined) out.push({ model, canonical: alias.canonical, label: alias.label, builtin: alias.builtin })
    seen.add(model)
  }
  for (const model of Object.keys(aliasOverrides)) {
    if (seen.has(model)) continue
    out.push({ model, canonical: aliasOverrides[model].canonical, label: aliasOverrides[model].label, builtin: false })
  }
  return out
}
function canonicalModel(model) {
  const alias = resolveAlias(model)
  return alias !== undefined ? alias.canonical : model
}
...
function offRow(model) {
  ...
  const base = OFFICIAL[canon] !== undefined ? OFFICIAL[canon] : null
...
function newAccount() {
  return {
    ...
    calls: { priced: 0, unpriced: 0, nonDeep: 0, aliased: 0 },
...
function applySample(acc, s, sign) {
  ...
  if (s.kind === 'priced') {
    acc.calls.priced += k
    if (resolveAlias(s.model) !== undefined) acc.calls.aliased += k
    acc.cost[s.phase] += k * s.cost.total
    let m = acc.byModel.get(s.model)
    if (m === undefined) { m = { inMiss: 0, inHit: 0, out: 0, peakCalls: 0, valleyCalls: 0, peakCny: 0, valleyCny: 0, aliased: 0 }; acc.byModel.set(s.model, m) }
    m.inMiss += k * s.tokens.inMiss
    m.inHit += k * s.tokens.inHit
    m.out += k * s.tokens.out
    if (resolveAlias(s.model) !== undefined) m.aliased += k
...
function mergeAccount(out, acc, id) {
  ...
  out.calls.aliased += acc.calls.aliased
  ...
  for (const [model, m] of acc.byModel) {
    ...
    if (t === undefined) { t = { inMiss: 0, inHit: 0, out: 0, peakCalls: 0, valleyCalls: 0, peakCny: 0, valleyCny: 0, aliased: 0 }; out.byModel.set(model, t) }
...
async function aggregate(ctx, rootId, diag) {
  const out = { tokens: { inMiss: 0, inHit: 0, out: 0, reasoning: 0 }, calls: { priced: 0, unpriced: 0, nonDeep: 0, aliased: 0 }, nonDeepModels: new Map(), cost: { peak: 0, valley: 0 }, byModel: new Map(), sessions: [] }
...
class DeepseekBillingService extends TypertRemoteService {
  ...
  async summary(input) {
    ...
    if (agg) {
      for (const [model, m] of agg.byModel) {
        const alias = resolveAlias(model)
        byModel.push({ model, label: (alias && alias.label) || LABELS[model] || model, aliased: (m.aliased || 0) > 0, inMiss: m.inMiss, inHit: m.inHit, out: m.out, peakCalls: m.peakCalls, valleyCalls: m.valleyCalls, peakCny: m.peakCny, valleyCny: m.valleyCny, cny: m.peakCny + m.valleyCny })
      }
    ...
    const resp = {
      ...
      calls: agg ? agg.calls : { priced: 0, unpriced: 0, nonDeep: 0, aliased: 0 },
      nonDeepModels,
  ...
  /** 别名通道管理（v0.1.20）：覆盖/添加/移除别名行，reset 恢复内置默认。 */
  async setAlias(input) {
    const a = input || {}
    if (a.reset === true) {
      for (const k of Object.keys(aliasOverrides)) delete aliasOverrides[k]
      aliasRemovals.clear()
      return { ok: true }
    }
    if (typeof a.model !== 'string' || a.model.trim() === '') return { ok: false }
    const model = a.model.trim()
    if (a.remove === true) {
      aliasRemovals.add(model)
      delete aliasOverrides[model]
      return { ok: true }
    }
    if (typeof a.canonical !== 'string' || a.canonical.trim() === '') return { ok: false }
    /* 目标 canonical 允许是另一个别名（自动解析到官方模型），但必须拒绝自环。 */
    const target = canonicalModel(a.canonical.trim())
    if (target === model) return { ok: false }
    aliasOverrides[model] = {
      canonical: target,
      label: typeof a.label === 'string' && a.label !== '' ? a.label : undefined
    }
    aliasRemovals.delete(model)
    return { ok: true }
  }
}
