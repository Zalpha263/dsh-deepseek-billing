...
/* ===== 别名通道：同源 DeepSeek 模型经代理/适配层暴露的 ID（v0.1.18） =====
   例：chat-ecnu（ECNU 校园代理，settings 中 ecnu-max 的 name 即 Deepseek-V4-Flash）。
   不在表内的模型仍按「非 DeepSeek」处理；也可用 setPrices 手动补价（见 offRow）。 */
const ALIASES = Object.create(null)
ALIASES['ecnu-max'] = 'deepseek-v4-flash' // Deepseek-V4-Flash @ ECNU
ALIASES['ecnu-image'] = 'deepseek-v4-flash-vision-exp' // Vision 变体 @ ECNU
const ALIAS_LABELS = Object.create(null)
ALIAS_LABELS['ecnu-max'] = 'V4 Flash · ECNU'
ALIAS_LABELS['ecnu-image'] = 'V4 Flash Vision · ECNU'
function canonicalModel(model) {
  return ALIASES[model] !== undefined ? ALIASES[model] : model
}
...
function offRow(model) {
  ...
  if (base === null) {
    /* 纯手动覆盖（无官方/别名行）：未设置的桶按 0 计，避免 NaN；全部未设置视为无价行。 */
    const mk = (bucket) => {
      if (ov[bucket] === undefined) return [0, 0]
    ...
    return (row.hit[0] === 0 && row.miss[0] === 0 && row.out[0] === 0) ? null : row
  }
  const mk = (bucket) => {
    /* ov 可能为 undefined（该模型无任何覆盖）——ov[bucket] 会在 undefined 上直接抛错，
       必须先判 ov 本身，否则每次官方价计价都会崩（旧代码靠 if (!ov) return base 兜底）。 */
    if (ov === undefined || ov[bucket] === undefined) return base[bucket]
    const v = Number(ov[bucket])
...
function applySample(acc, s, sign) {
  ...
  if (s.kind === 'priced') {
    acc.calls.priced += k
    if (ALIASES[s.model] !== undefined) acc.calls.aliased += k
    ...
    if (ALIASES[s.model] !== undefined) m.aliased += k
...
class DeepseekBillingService extends TypertRemoteService {
  ...
  async summary(input) {
    ...
    const meta = { priceSource: 'official-2026-09-03', fxRate, fxSource, updatedAt: now }
    if (rootId === null) {
      return {
    ...
    if (agg) {
      for (const [model, m] of agg.byModel) {
        byModel.push({ model, label: ALIAS_LABELS[model] || LABELS[model] || model, aliased: (m.aliased || 0) > 0, inMiss: m.inMiss, inHit: m.inHit, out: m.out, peakCalls: m.peakCalls, valleyCalls: m.valleyCalls, peakCny: m.peakCny, valleyCny: m.valleyCny, cny: m.peakCny + m.valleyCny })
      }
