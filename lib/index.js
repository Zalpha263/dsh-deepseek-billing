...
/** 折叠一个子代理会话的持久化日志；按（子 id, 事件数）缓存，变化时重折。 */
async function childAccount(ctx, childId) {
  const sp = ctx.get('sessionPersistence')
  if (sp === undefined || typeof sp.readFrom !== 'function') return null
  let events = []
  let len = 0
  try {
    const res = await sp.readFrom(childId, 0)
    events = (res && res.events) || []
    len = events.length
  } catch (err) {
    const cached = childAccounts.get(String(childId))
  ...
  childAccounts.set(String(childId), { len, acc })
  return acc
}
...
async function aggregate(ctx, rootId) {
  ...
  for (const cid of children) {
    const acc = await childAccount(ctx, cid)
