...
async function persistedChildren(ctx, rootId) {
  ...
  while (queue.length > 0) {
    ...
    for (const entry of headers) {
      /* 0.1.5-rc.2: list() yields SessionPersistenceSnapshot{header, revision};
         <=0.1.2-rc.1 yielded the bare header. Accept both shapes. */
 ...
 *  0.1.5-rc.2：SessionPersistence 已无 readFrom()；优先用 live-preferred 的
 *  ctx.sessionQuery.readSession(id)（返回 {session, events}），否则用句柄
 *  open(id,'read') + read(0) 兜底；<=0.1.2-rc.1 仍保留 readFrom()。
...
class DeepseekBillingService extends TypertRemoteService {
   ...
   *  面板的窗口费用不会跟着变。
   *  v0.1.28：现代字段传 value = null/'' 表示清除该字段的手动覆盖（回到内置价）。 */
  async setPrices(input) {
    ...
    /* v0.1.28：现代六值接口里 value = null/'' 表示「清除该字段的手动覆盖，回到
       内置价」。此前客户端对空输入直接跳过，宿主覆盖值永远留着——表单显示内置价、
       实际仍按旧手动价计费。 */
    if (modernField && (a.value === null || a.value === undefined || a.value === '')) {
